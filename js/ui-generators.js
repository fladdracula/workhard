  ...
  createIcon("game", 32, 64 * 5, function () {
    if (state == STATE.BREAK) {
      ...
      createSimpleDialog(
        "Uh-oh!",
        "breakout-forbidden",
        "You can't play Breakout unless you're on a break!",
        "Okay",
        true,
        "400"
      );
...
function createIcon(name, x, y, callback) {
  ...
  var div = $(
    '<div class="icon"><img src="assets/images/icons/' +
      name +
      '.png"><div class="icon-label">' +
      label +
      "</div></div>"
  );
...
function createBreakoutDialog() {
  ...
  var dialogDiv = $(
    '<div class="dialog" id="breakout-dialog" title="' + title + '"></div>'
  );
...
function createMenuBar() {
  ...
  // rank = getNextRank();
  var menubarJobTitle = $(
    '<div id="menubar-job-title"><b>Job Title:</b> <span id="menubar-job-title-text">' +
      jobTitle +
      "</span></div>"
  );
  menubarJobTitle.css({
  ...
  var menubarUnits = $(
    '<div id="menubar-units"><b>Work Units to Promotion:</b> <span id="menubar-units-text">' +
      workUnitsToPromotion +
      "</span></div>"
  );
  menubarUnits.css({
    // left: '33%',
...
function createLoginDialog() {
  var title = "Login";
  var dialogDiv = $(
    '<div class="dialog" id="login-dialog" title="' + title + '"></div>'
  );
  var usernameField = $(
    '<input id="usernameInput" type="text" maxlength="10"></input>'
  );
...
function createReadyDialog() {
  ...
  var dialogDiv = $(
    '<div class="dialog" id="ready-dialog" title="' + title + '"></div>'
  );
...
function createDelayedWorkDialog() {
  var title = "Get Ready!";
  var dialogDiv = $(
    '<div class="dialog" id="work-delay-dialog" title="' + title + '"></div>'
  );
...
function createBreakDialog() {
  ...
  var dialogDiv = $(
    '<div class="dialog" id="break-dialog" title="' + title + '"></div>'
  );
...
function createDesktopDialog() {
  ...
  var dialogDiv = $(
    '<div id="desktop-dialog" class="dialog" title="' + title + '"></div>'
  );
  ...
  for (var i = 0; i < options.length; i++) {
    ...
    var radio = $(
      '<input id="' +
        name +
        '" class="radio" name="desktop-select" type="radio">'
    );
...
function createMusicDialog() {
  ...
  var dialogDiv = $(
    '<div id="music-dialog" class="dialog" title="' + title + '"></div>'
  );
  ...
  for (var i = 0; i < options.length; i++) {
    ...
    var radio = $(
      '<input id="' + name + '" class="radio" name="music-select" type="radio">'
    );
    radio.data("label", options[i]);
    radio.click(function () {
...
function createInspirationalDialog() {
  ...
  var title =
    inspirationWorkSlogans[_.random(0, inspirationWorkSlogans.length - 1)];
  var dialogDiv = $(
    '<div class="dialog inspirational-dialog" title="' + title + '"></div>'
  );
...
function createWorkDialog() {
  ...
  var dialogDiv = $(
    '<div class="dialog work-dialog" title="' + title + '"></div>'
  );
  ...
  for (option = 1; option <= numSteps; option++) {
    ...
    switch (type) {
      ...
      case TYPE.SELECTMENU:
        ...
        var selectString =
          "<p>" + option + ". Select " + selectMenu.data("correct") + "</p>";
      ...
      case TYPE.CHECKBOX:
        ...
        for (var i = 0; i < checkboxes.data("correct").length; i++) {
          ...
          if (i == checkboxes.data("correct").length - 2)
            selectString += " and ";
      ...
      case TYPE.RADIO:
        ...
        var selectString =
          "<p>" + option + ". Select " + radios.data("correct") + "</p>";
      ...
      case TYPE.PROGRESSBAR:
        ...
        var instruction =
          "<p>" + option + ". Wait for progress bar to complete</p>";
      ...
      case TYPE.DATEPICKER:
        ...
        var dateString =
          ...
          date.getFullYear();
        var instruction =
          "<p>" + option + ". Set date to " + dateString + "</p>";
  ...
  dialogDiv.append(
    "<p>" + option + ". Click " + buttonsArray[correctButton] + "</p>"
  );
...
function createWindowSizeDialog() {
  var d = createSimpleDialog(
    "Window too small!",
    "window-size-dialog",
    "<p>You can't get any work done with a browser width that narrow!</p><p>Make it bigger!</p>",
    null,
    false,
    "80%",
    true,
...
function createSimpleDialog(
  ...
  var div = $(
    '<div class="dialog simple-dialog" id="' +
      id +
      '" title="' +
      title +
      '">' +
...
function createDialog(div, options, random, stationary) {
  ...
  if (random) {
    ...
    var y = _.random(
      $(".menu-bar").height(),
      $(window).height() - div.parent().height()
    );
  ...
  div.parent().draggable({
    containment: [
      0,
      $(".menu-bar").height(),
      $(window).width() - div.parent().width() - 10,
      $(window).height() - div.parent().height(),
    ],
  });
...
function createCheckbox(_numOptions, _index) {
  ...
  for (var i = 0; i < _numOptions; i++) {
    ...
    if (
      Math.random() < 0.5 ||
      (correctElements.length == 0 && i == _numOptions - 1)
    ) {
...
function createDocumentDialog() {
  var title =
    inspirationWorkSlogans[_.random(0, inspirationWorkSlogans.length - 1)];
  var div = $(
    '<div class="dialog document-dialog" title="' + title + '"></div>'
  );
  var requiredCharacters = _.random(250, 750);
  var instruction1 = $(
    "<span>Write and save a document of at least " +
      requiredCharacters +
      " characters (currently </span>"
  );
  var charactersSpan = $("<span>0</span>");
  var instruction2 = $("<span>)</span>");
  var input = $(
    '<textarea class="document-input" style="user-select: none" readonly="readonly"></textarea>'
  );
  ...
  input.on("keypress", function (e) {
    ...
    $(this)
      .parent()
      .data("correct", characters >= requiredCharacters);
...
function createEmailDialog() {
  ...
  var instruction1 = $(
    "<span>Write and send an email of at least " +
      requiredCharacters +
      " characters (currently </span>"
  );
  var charactersSpan = $("<span>0</span>");
  ...
  var email = technologies[_.random(0, technologies.length - 1)]
    .replace(/ /g, ".")
    .toLowerCase();
  email +=
    "@" +
    technologies[_.random(0, technologies.length - 1)]
      .replace(/ /g, "")
      .toLowerCase();
  email += ".com";

  var to = $(
    '<span name="email-to-field" class="email-field">' + email + "</span>"
  );

  var subjectText = "Re: " + technologies[_.random(0, technologies.length - 1)];
  var subject = $(
    '<span name="email-subject-field" class="email-field">' +
      subjectText +
      "</span>"
  );
  var input = $(
    '<textarea class="email-input" style="user-select: none" readonly="readonly"></textarea>'
  );
  ...
  div.append("<p>");
  div.append(
    '<label for="email-to-field" class="email-field-label">To:</label>',
    to
  );
  // div.append('<p>')
  div.append(
    '<label for="email-subject-field" class="email-field-label">Subject:</label>',
    subject
  );
  div.append("<p></p>");
  div.append(input);
...
function givePromotion() {
  ...
  if (!jobTitle)
    jobTitle = getJobTitle(jobTitleSubjectIndex, jobTitlePositionIndex);
...
function resetWorkUnits() {
  if (jobTitle == "Chief Technology Officer") {
    ...
    console.log(jobTitleSubjectIndex, jobTitlePositionIndex);
    WORK_UNITS_FOR_PROMOTION =
      (jobTitleSubjectIndex + 1) * 1500 + jobTitlePositionIndex * 10000;
    WORK_UNITS_PER_DIALOG =
      (jobTitleSubjectIndex + 1) * 50 + jobTitlePositionIndex * 450;
    WORK_UNITS_PER_CHARACTER =
      (jobTitleSubjectIndex + 1) * 2 + jobTitlePositionIndex * 15;
  }
...
function createPromotionDialog(newJobTitle) {
  var dialogDiv = $(
    '<div class="dialog promotion-dialog" title="Promotion!"></div>'
  );
  dialogDiv.append(
    "<p>You have been promoted to <b>" + newJobTitle + "</b></p>"
  );
